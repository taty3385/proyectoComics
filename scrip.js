...
const getMarvelData = async (resource,offset,limit) => {
    const url = `${urlBase}${resource}?${ts}${keyPublic}${hash}&offset=${offset}&limit=${limit}`;
...
const printComic = async () => {
    const comicsData = await getMarvelData("comics",offset,limit);
...
const printCharacter = async () => {
    const charactersData = await getMarvelData("characters",offset,limit);
    $(".containerResult").innerHTML = "";
...
const printContent = async (resource,offset,limit) => {
    if (resource === 'comics' || resource === 'characters') {
        const contentData = await getMarvelData(resource,offset,limit);
...
$("#optionPersonajes").addEventListener("change", () => {
    ...
    printContent(valueOption,offset,limit);
});
